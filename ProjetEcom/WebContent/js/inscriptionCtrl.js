		...
		.controller(
				...
				function($scope, $http) {
					...
					$scope.submit = function() {
						...
						// Teste si l'un des champs contient des caractères
						// interdits
						if (checkCaracteresInterdits(this._id)
								|| checkCaracteresInterdits(this._nom)
								|| checkCaracteresInterdits(this._prenom)
								|| checkCaracteresInterdits(this._mdp)
								|| checkCaracteresInterdits(this._mdpc)) {
							document
									.getElementById('erreurCaracteresInterdits').style.display = "block";
						} else {
							// Teste si le mot de passe est trop court
							if (this._mdp.length < 8 || this._mdpc.length < 8) {
								document
										.getElementById('erreurMotDePasseTropCourt').style.display = "block";
							} else {
								...
								if (!validationEmail(this._email)) {
									...
									$http.get("InscriptionClient", {params : {
															"identifiant" : this._id,
															"nom" : this._nom,
															"prenom" : this._prenom,
															"motdepasse" : this._mdp,
															"motdepasseconfirm" : this._mdpc,
															"email" : this._email
														}}).then(function(response) {
														$http.get(
																		"ConfirmationInscriptionServlet",
																		{
																			params : {
																				"id" : this._id
																			}
